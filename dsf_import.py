...
if 0 <= AREA_W <= 1 and 0 <= AREA_S <= 1: 
    AREA_W += grid_west
    AREA_E += grid_west
    AREA_S += grid_south
    AREA_N += grid_south
print("But extracting just from west {} to east {} and south {} to north {}".format(AREA_W, AREA_E, AREA_S, AREA_N))
...
for id in dsf.DefTerrains:
    terrain_details[id] = read_ter_file(dsf.DefTerrains[id], xp_path, dsf_file)
...
for ter_layer_id in sorted(ter_layers.keys()):      
    ...
    for p in ter_layers[ter_layer_id]:
        ...
        for t in trias:
            if not (AREA_W <= dsf.V[t[0][0]][t[0][1]][0] <= AREA_E and AREA_S <= dsf.V[t[0][0]][t[0][1]][1] <= AREA_N)  \
                and not (AREA_W <= dsf.V[t[1][0]][t[1][1]][0] <= AREA_E and AREA_S <= dsf.V[t[1][0]][t[1][1]][1] <= AREA_N) \
                and not (AREA_W <= dsf.V[t[2][0]][t[2][1]][0] <= AREA_E and AREA_S <= dsf.V[t[2][0]][t[2][1]][1] <= AREA_N):
                    continue
...
for ter_layer_id in materials:
    ...
    if ter_layer_id[0] > 1:  # this is an overlay
        terrain_name += "_O"
    m = add_material(terrain_name, terrain_details[ter_layer_id[1]], bpy)  # add material to Blender materials
